...
<body class="bg-gray-900 text-white min-h-screen flex items-center justify-center">
    <div class="max-w-md w-full mx-auto bg-white text-black rounded-3xl shadow-2xl p-10 animate-in">
        <h1 class="text-3xl font-bold text-center mb-8 text-red-600">Login Page (SQL Injection Vulnerable)</h1>
        ...
        <form method="post" class="space-y-6">
            <div>
                ...
                <input type="text" name="username" class="w-full px-5 py-4 border border-gray-300 rounded-2xl focus:outline-none focus:ring-4 focus:ring-red-300 transition" placeholder="admin">
            </div>
            <div>
                <label class="block text-sm mb-2">Password</label>
                <div class="relative">
                    <input type="password" id="password" name="password" 
                           class="w-full px-5 py-4 border border-gray-300 rounded-2xl focus:outline-none focus:ring-4 focus:ring-red-300 transition" placeholder="password123">
                    <button type="button" onclick="togglePassword()" 
                            class="absolute right-4 top-1/2 -translate-y-1/2 text-gray-500 hover:text-gray-700 text-2xl">Show</button>
                </div>
            </div>
            <button type="submit" name="login" 
                    class="w-full bg-red-600 hover:bg-red-700 text-white py-4 rounded-2xl font-bold text-lg transition transform hover:scale-105 active:scale-95">
                Login
            </button>
        </form>
